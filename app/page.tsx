...
const MAIN_MENU_OPTIONS: MenuOption[] = [
  ...
  },
  {
    ...
    value: "training",
    description:
      "See membership training, membership programs, and the video training library.",
  },
];

function isIsolatedContext(ctx: string): boolean {
  if (!ctx) return false;
  return ISOLATED_CTX_PREFIXES.some((prefix) => ctx.startsWith(prefix));
}

function extractCtx(content: string): string {
...
function removeNextPrompt(content: string): string {
  return content
    .replace(
      /\*\*what would you like (to do next|to explore next|next)\?\*\*/gi,
      ""
    )
...
function parseMenuItems(content: string, ctx: string): MenuItem[] {
  ...
  if (ctx.startsWith("quiz>fit>") || ctx.startsWith("quiz>membership>")) {
    ...
    for (const line of lines) {
      ...
      if (["1", "2", "3", "4"].includes(displayKey)) {
        items.push({
          key: displayKey,
          label,
          displayKey,
  ...
  const dividerIdx = content.lastIndexOf("\n---\n");
  if (dividerIdx === -1) return [];

  const menuBlock = content.slice(dividerIdx);
  const menuLines = menuBlock.split("\n");
  const items: MenuItem[] = [];

  const labelToCommand = new Map<string, string>([
    ["Explore CIBN", "explore cibn"],
    ["I'm a Member", "i'm a member"],
    ["Tools", "tools"],
    ["Training", "training"],
    ["Member Login", "member login"],

    ["What is CIBN?", "what is cibn"],
    ["Is CIBN a Good Fit for Me?", "is cibn a good fit for me"],
    ["Membership Options", "membership options"],

    ["Types of Membership", "types of membership"],
    ["Take the Membership Quiz", "take the membership quiz"],
    ["Take the CIBN Fit Quiz", "take the cibn fit quiz"],

    ["Getting Started", "getting started"],
    ["Networking Flows", "networking flows"],
    ["Networking Strategies", "networking strategies"],

    ["Membership Training", "membership training"],
    ["Membership Programs", "membership programs"],
    ["Video Training Library", "video training library"],
  ]);
  ...
  for (const line of menuLines) {
    ...
    const key =
      isIsolatedContext(ctx) ? displayKey : labelToCommand.get(label) ?? displayKey;

    items.push({ key, label, displayKey });
  }

  return items;
}

function stripListLines(content: string, ctx?: string): string {
  const lines = content.split("\n");

  if (ctx && (ctx.startsWith("quiz>fit>") || ctx.startsWith("quiz>membership>"))) {
    const kept: string[] = [];
    ...
    for (const line of lines) {
      ...
      if (trimmed === "---") continue;
      if (trimmed === "**Membership Options**") continue;
      if (trimmed === "**Explore CIBN**") continue;
      if (trimmed === "**Main Menu**") continue;
      if (trimmed === "Please reply with the number of your choice.") continue;
  ...
  const dividerIdx = content.lastIndexOf("\n---\n");
  const displayPart = dividerIdx !== -1 ? content.slice(0, dividerIdx) : content;

  return displayPart.replace(/\n{3,}/g, "\n\n").trim();
}
...
export default function Page() {
  ...
  const latestAssistantRaw =
  [...messages].reverse().find((m) => m.role === "assistant")?.content ?? "";

const latestAssistantCtx = extractCtx(latestAssistantRaw);

const activeQuizMode =
  latestAssistantCtx.startsWith("quiz>fit>") ||
  latestAssistantCtx.startsWith("quiz>membership>");
        ...
        {MAIN_MENU_OPTIONS.map((item) => {
          ...
          return (
            <button
              key={item.value}
              type="button"
              onClick={() => startFreshTopic(item.value, item.label)}
              style={{
                width: "100%",
                textAlign: "left",
                background: isActive
  ...
  const rawContent = messages[msgIndex]?.content ?? message.content;
  const ctx = extractCtx(rawContent);
  ...
  if (activeQuizMode && msgIndex !== visibleMessages.length - 1) {
    return null;
  }
  ...
  const menuItems =
    message.role === "assistant" ? parseMenuItems(rawContent, ctx) : [];

  const isQuizContext =
    ctx.startsWith("quiz>fit>") || ctx.startsWith("quiz>membership>");

  const showButtons = isLastAssistant && menuItems.length > 0 && !isLoading;

  const displayContent =
    showButtons || isQuizContext
      ? stripListLines(message.content, ctx)
      : message.content;
